...
def interleave_lists(list1, list2):
    assert len(list1) == len(list2), "두 리스트의 길이가 같아야 합니다."
...
def main():
    ...
    parser.add_argument('--erpa', type=bool, default=False)
    ...
    for image_id, val_data in enumerate(val_loader):
        ...
        lq_tensor = F.interpolate(lq_tensor.unsqueeze(0), scale_factor=args.upsample_size, mode='bilinear', align_corners=False).squeeze()
        gt_tensor = F.interpolate(gt_tensor.unsqueeze(0), scale_factor=args.upsample_size, mode='bilinear', align_corners=False).squeeze()
        ...
        for i, quad in enumerate(lq_quads):
            ...
            data = {'LQ': lq, 'GT': gt # LQ 무쓸모
            }

            model.feed_data(data)
            model.test(image_id * 1000 + i)
            visuals = model.get_current_visuals()
            LR = visuals.get('LR')[0].squeeze() # 워터마크 들어간 애

            orig_msg = visuals['message'][0]
            rec_msg = visuals['recmessage'][0]
            original_messages.append(orig_msg)
            diff_quads.append(bit_difference(orig_msg.squeeze(), rec_msg.squeeze()))


            embedded_quads.append(LR)
            # embedded_quads.append(visuals['SR'][0])
        ...
        for i, quad in enumerate(lq_quads):
            ...
            model.feed_data(data)
            model.test(image_id * 1000 + i, m=diff_quads[(i-1)//2])
            visuals = model.get_current_visuals()
            LR = visuals.get('LR')[0].squeeze() # 워터마크 들어간 애
            bit_repair_quads.append(LR)

        quads = interleave_lists(embedded_quads, bit_repair_quads)
        composed = compose_image_tensor(quads, args.grid_size)
        composed_img = util.tensor2img(composed)
        util.save_img(composed_img, f'results/composed_{image_id}.png')

        bers = [bit_error_rate_tensor(m1, m2)
                for m1, m2 in zip(original_messages, recovered_messages)]
        
        


        for i, ber in enumerate(bers):
            print(f"Image {image_id} Quad {i}: BER = {ber*100:.4}%")
